# Hello ~~darkness my old friend~~ there

My name is Andrey. Gradient descent believer. Engineer at heart.
Having fun with [ML research puzzles](https://scholar.google.com/citations?user=gVWSh9kAAAAJ) - trying to understand why models are the way they are and then make them smarter. Enjoying a bit of storytelling on the side ([blog](https://blog.faillearnrepeat.net/), [talks](https://github.com/fxlrnrpt/talks)).

Fun facts: lived in a recording studio in a former bomb shelter (even [recorded some music](https://music.faillearnrepeat.net/)); have three tattoos quoting Dostoevsky in one way or another; always follow the irrational urge to put a piano in every place I call home.

Living in: London ([UK Global Talent alumnus](https://github.com/fxlrnrpt/uk-global-talent-visa-guide)).

[LinkedIn](https://www.linkedin.com/in/faillearnrepeat/), [X](https://x.com/fxlrnrpt), [CV](https://faillearnrepeat.net/cv), [email](mailto:andrey@faillearnrepeat.net).
